...
@router.post("/", response_model=CommentRead, status_code=status.HTTP_201_CREATED, summary="새로운 댓글 작성")
def create_new_comment(post_id: int, comment_in: CommentCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Comment:
    """
...
def update_existing_comment(comment_id: int, comment_in: CommentUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Comment:
    """
    기존 댓글 내용을 수정합니다.
...
@router.delete("/{comment_id}", status_code=status.HTTP_204_NO_CONTENT, summary="기존 댓글 삭제")
def delete_existing_comment(comment_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    특정 댓글을 삭제합니다.
